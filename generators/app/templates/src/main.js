...
import Checkbox from 'vendor/checkbox';
import CheckboxButton from 'vendor/checkbox-button';
import CheckboxGroup from 'vendor/checkbox-group';
import VDialog from 'vendor/dialog';
import mixin from './mixin';
import './../static/plugins/animate.css';
import './scss/reset-element.scss';
...
Vue.prototype.$ELEMENT = {size: '', zIndex: 3000}; //  large / mini medium / small / mini
...
/**
 * store.state.route.path   | this.$store.route.path         // current path (string)
 * store.state.route.params |this.$store.route.params   // current params (object)
 * store.state.route.query  | this.$store.route.query   // current query (object)
 * */
Vue.mixin(mixin);
sync(store, router);
...
Vue.use(CountTo);
// 延时加载
Vue.use(VueLazyload, { // eg: <img v-lazy="img">
    preLoad: 1.3,
    error: require('./assets/images/image-empty.png'),
