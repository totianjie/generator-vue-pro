...
const URL = {
    ...
    // 疑点审核: 正常提交（按顺序）
    getNormalSubmit: '/process/records/normal',
    // 疑点审核: 转向提交（转向下一节点）
    getTurn: '/process/records/turn'
};

export function getVisitInfo (data) {
    let url = URL.getVisitInfo + '/' + data.jzcode;
...
export function getCaseInfoList (data) {
    let url = URL.getCaseInfoList + '/' + data.jzcode;
    return ajax(url, {data, method: 'post'});
}
...
export function getNormalSubmit (data) {
    let url = URL.getNormalSubmit;
    return ajax(url, {data, method: 'post'});
}
export function getTurn (data) {
    let url = URL.getTurn;
    return ajax(url, {data, method: 'post'});
}
/** 病例展示 end */
