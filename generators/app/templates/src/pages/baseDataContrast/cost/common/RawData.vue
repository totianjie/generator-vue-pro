        ...
        methods: {
            ...
            _audit () { // 审核
                ...
                if (this.isSelection() === 0) {
                    return false;
                } else if (this.isSelection() > 0) {
                    this.rawDataList.forEach((item, index) => {
                        if (Number(item.status) === 0) { // 0：未对照 1已对照未审核 2已审核 3已撤销 4不能对照 ,
                            this.$toast('只有对照好的数据才可做审核');
                            return false;
                        }
                        if (Number(item.status) === 2) { // 0：未对照 1已对照未审核 2已审核 3已撤销 4不能对照 ,
                            this.$toast('您好！该记录已审核，请不要重复操作，谢谢！');
                            return false;
                        }
                        if (!item.auditId) {
            ...
            _cancelAudit () { // 撤销审核
                ...
                if (this.isSelection() === 0) {
                    ...
                    this.rawDataList.forEach((item, index) => {
                        if (!Number.isNaN(item.status) && Number(item.status) !== 2) {
                ...
                // 撤销  必需选择一个原始数据(接口数据格式还没改)
                cancelAudit(dataArr).then(res => {
                    this._callBackFn();
                });
            },
            _submit () { // 提交
                ...
                if (this.isSelection() === 0) {
                    ...
                    this.rawDataList.forEach((item, index) => {
                        if (Number.isNaN(item.status) || Number(item.status) !== 0) { // 0：未对照 1已对照未审核 2已审核 3已撤销 4不能对照 ,
                            this.$toast('该数据状态不支持“提交”操作。');
                            return false;
                        }
                        if (!item.catalogName) { // 0：未对照 1已对照未审核 2已审核 3已撤销 4不能对照 ,
            ...
            isSelection () {
                ...
                if (this.rawDataList.length === 0) {
                    ...
                    this.$toast('请选择原始数据1');
                    return false;
                } else if (this.rawDataList.length === 1) {
                    statusNum = 1;
                } else if (this.rawDataList.length > 1) {
                    statusNum = 2;
                }
