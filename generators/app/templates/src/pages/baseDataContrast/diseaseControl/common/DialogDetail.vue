...
<template>
    ...
    >
        ...
        <div style="padding: 0 10px; max-height: 400px; overflow: auto;">
            ...
            <v-item-detail :tooltip="true" :pxUnit="true" :labelWidth="labelWidth" label="原始诊断编码：" :span="span">
                <span class="block text-ellipsis">{{detailDialogData.instDiseaseCode}}</span>
                <div slot="content" class="tooltip-item">{{detailDialogData.instDiseaseCode}}</div>
            </v-item-detail>
            <v-item-detail :tooltip="true" :pxUnit="true" :labelWidth="labelWidth" label="原始诊断名称：" :span="span">
                <span class="block text-ellipsis">{{detailDialogData.instDiseaseName}}</span>
                <div slot="content" class="tooltip-item">{{detailDialogData.instDiseaseName}}</div>
            </v-item-detail>
            ...
            <v-item-detail :tooltip="true" :pxUnit="true" :labelWidth="labelWidth" label="主要诊断：" :span="span">
...
<script>
    ...
    export default {
        ...
        computed: {
            list () {
                let {diseaseCode, diseaseCodeAdd, diseaseName} = this.detailDialogData;
                if (!diseaseName) {
                    return [];
                }
                let diseaseCodeArr = diseaseCode.split(';');
                let diseaseCodeAddArr = diseaseCodeAdd.split(';');
                return diseaseName.split(';').map((item, index) => {
                    return {
                        diseaseName: item,
                        diseaseCodeAdd: diseaseCodeAddArr[index],
                        diseaseCode: diseaseCodeArr[index],
                        index
                    };
                });
            }
        ...
        methods: {
            ...
            handlerDelete (row) { // 表格删除
                console.log(row);
                this.flag = true;
                let {diseaseHeadVersion, diseaseHeadVersionName, id} = this.detailDialogData;
                let params = {
                    diseaseCode: '',
                    diseaseCodeAdd: '',
                    diseaseName: '',
                ...
                this.list.forEach((item, index) => {
                    ...
                    params.diseaseCode += `${symbol}${item.diseaseCode}`;
                    params.diseaseCodeAdd += `${symbol}${item.diseaseCodeAdd}`;
                    params.diseaseName += `${symbol}${item.diseaseName}`;
                });
                console.log(params);
