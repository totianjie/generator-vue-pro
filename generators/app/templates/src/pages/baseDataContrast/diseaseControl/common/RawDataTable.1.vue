...
<template>
    <div class="w-fill">
        ...
        >
            ...
            >
                <template slot-scope="scope" v-if='item.prop===`instDiseaseName`'>
                    <span @click="handleDetailNameClick(scope.row)">{{scope.row.instDiseaseName}}</span>
                </template>
             </el-table-column>
            <!-- <el-table-column
                prop="serialNum"
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapGetters(['diseaseControlLeft'])
        },
        watch: {
            diseaseControlLeft: function (newVal, oldVal) {
                console.log(`数据变化了 >>>>>`, newVal);
                this.tableDataRaw = [...newVal];
            }
        },
        created () {
            let localRaw = window.localStorage.getItem("diseaseControl");
            localRaw = localRaw ? JSON.parse(localRaw).raw : [...tableDataRaw.DISEASE_CONTROL.raw];
            this.tableDataRaw = [...localRaw];
            console.log(`localRaw >>>>`, localRaw);
        },
        methods: {
            ...mapMutations(['setdiseaseControlLeft']),
            ...mapActions(['addData']),
            __show (item, index) {
                let that = this;
                switch (item.prop) {
            ...
            handleDetailNameClick (row) { // 原始诊断名称
                busEvent.$emit('baseDataContrast/diseaseControl/common/RawDataTableMixins.rowClick', row);
            }
        },
