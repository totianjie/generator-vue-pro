            ...
            <div style="max-height: 350px; overflow-y: auto;">
                ...
                <v-item-detail class="item-detail" :border="false" :pxUnit="true" :labelWidth="labelWidth" label="原始名称：" :span="span">
                    <ul class="dialog-list">
                        <li class="dialog-item" v-for="(item, index) in rawDataList" :key="index" :title="item.instOperationName" v-text="item.instOperationName"></li>
                    </ul>
                </v-item-detail>
                <v-item-detail class="item-detail" :border="false" :pxUnit="true" :labelWidth="labelWidth" label="标准名称：" :span="span">
                    <ul class="dialog-list">
                        <li class="dialog-item" v-for="(item, index) in standardDataList" :key="index" :title="item.operationName" v-text="item.operationName"></li>
                    </ul>
                </v-item-detail>
            </div>
...
<script>
    ...
    } from "js/api/baseDataContrast/operation";
    ...
    export default {
            ...
            searchParams: {
                ...
                default () {
                    return { // 查询的参数
                        compareOptUser: '', // 对照人
                        batchNo: '', //  批次号
                        instOperationCode: '', // 原始疾病编码
                        instOperationName: '', // 原始疾病名称
                        operationName: '', // 标准疾病名称
                        startTime: '', // 筛选条件-起始时间
                        endTime: '' // 筛选条件-结束时间
                    };
                }
        ...
        methods: {
            ...
            _uniq (oldList, newList) { // 去重
                let newArr = oldList.map(({id, operationCode, operationName}) => { // 原始数据整理，准备做去重
                    operationCode = operationCode === null ? '' : operationCode;
                    operationName = operationName === null ? '' : operationName;
                    let codeList = operationCode.split(';');
                ...
                newList.forEach(({operationHeadVersion, operationHeadVersionName, operationCode, operationName}) => {
                    operationCode = operationCode === null ? '' : operationCode;
                    operationName = operationName === null ? '' : operationName;
                    let keys = operationCode;
                    newArr.forEach((item, index) => {
                        if (!item.ids.includes(keys)) {
                            item.operationHeadVersion = operationHeadVersion;
                            item.operationHeadVersionName = operationHeadVersionName;
                            item.operationCode = this._strJoin(item.operationCode, operationCode);
                            item.operationName = this._strJoin(item.operationName, operationName);

                            if (item.operationCode.charAt(0) === ';' && item.operationName.charAt(0) === ';') {
                                item.operationCode = item.operationCode.substring(1);
                                item.operationName = item.operationName.substring(1);
                            }
                        } else {
                            console.log('存在');
                        }
            ...
            _submit () { // 提交
                ...
                if (!this.rawData.operationName) { // 0：未对照 1已对照未审核 2已审核 3已撤销 4不能对照 ,
