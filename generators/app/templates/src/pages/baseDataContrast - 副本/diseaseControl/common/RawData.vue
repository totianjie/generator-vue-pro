            ...
            <div style="max-height: 350px; overflow-y: auto;">
                ...
                <v-item-detail class="item-detail" :border="false" :pxUnit="true" :labelWidth="labelWidth" label="原始名称：" :span="span">
                    <ul class="dialog-list">
                        <li class="dialog-item" v-for="(item, index) in rawDataList" :key="index" :title="item.instDiseaseName" v-text="item.instDiseaseName"></li>
                    </ul>
                </v-item-detail>
                <v-item-detail class="item-detail" :border="false" :pxUnit="true" :labelWidth="labelWidth" label="标准名称：" :span="span">
                    <ul class="dialog-list">
                        <li class="dialog-item" v-for="(item, index) in standardDataList" :key="index" :title="item.diseaseName" v-text="item.diseaseName"></li>
                    </ul>
                </v-item-detail>
            </div>
...
<script>
    ...
    import {
        ...
        synAuditDataByIds,
        matchingCatalog // 自动对照
    } from "js/api/baseDataContrast/diseaseControl";
    ...
    export default {
        ...
        mounted () {
            // 标准数据  checkbox选择变化
            busEvent.$on('baseDataContrast/diseaseControl/common/StandardData.standardDataList', (arr = []) => {
                console.log('标准数据多选 数据 变化', arr);
                this.standardDataList = arr;
            });
            // 标准数据  dbClick  双击
            busEvent.$on('baseDataContrast/diseaseControl/common/StandardData.dbClick', (json = null) => {
                console.log(this);
        ...
        methods: {
            ...
            _getTableData (
                ...
                if (oldSearchParams.batchNo) {
                    ...
                    return Promise.all([statistics({batchNo: oldSearchParams.batchNo}), getDiseaseRelationList(params)]).then(([dataCount, {data}]) => {
                        ...
                        console.log('fasfdasd', dataCount);
            ...
            _multitermMatch () { // 多项对照  单项对照
                ...
                /*
                diseaseCode (string, optional): 标准疾病编码，多个用英文（;）分割 ,
                diseaseCodeAdd (string, optional): 附加码(标准)，多个用英文（;）分割 ,
                diseaseHeadVersion (string, optional): 疾病版本id ,
                diseaseHeadVersionName (string, optional): 标准疾病版本名称 ,
                diseaseName (string, optional): 标准疾病名称，多个用英文（;）分割 ,
            ...
            _uniq (oldList, newList) { // 去重
                let newArr = oldList.map(({id, diseaseCode, diseaseCodeAdd, diseaseName}) => { // 原始数据整理，准备做去重
                    let codeList = diseaseCode.split(';');
                    let codeAddList = diseaseCodeAdd.split(';');

                    let ids = codeList.map((item, index) => {
                        let str = codeAddList[index] ? codeAddList[index] : '';
                        return item + str;
                    });
                    console.log('idsidsids', ids);
                    return {
                        diseaseHeadVersion: '',
                        diseaseHeadVersionName: '',
                        id,
                        diseaseCode,
                        diseaseCodeAdd,
                        diseaseName,
                ...
                newList.forEach(({diseaseHeadVersionId, diseaseHeadVersionName, diseaseCode, diseaseCodeAdd, diseaseName}) => {
                    let keys = diseaseCode + diseaseCodeAdd;
                    newArr.forEach((item, index) => {
                        if (!item.ids.includes(keys)) {
                            item.diseaseHeadVersion = diseaseHeadVersionId;
                            item.diseaseHeadVersionName = diseaseHeadVersionName;
                            item.diseaseCode = this._strJoin(item.diseaseCode, diseaseCode);
                            item.diseaseCodeAdd = this._strJoin(item.diseaseCodeAdd, diseaseCodeAdd);
                            item.diseaseName = this._strJoin(item.diseaseName, diseaseName);

                            if (item.diseaseCode.charAt(0) === ';' && item.diseaseCodeAdd.charAt(0) === ';' && item.diseaseName.charAt(0) === ';') {
                                item.diseaseCode = item.diseaseCode.substring(1);
                                item.diseaseCodeAdd = item.diseaseCodeAdd.substring(1);
                                item.diseaseName = item.diseaseName.substring(1);
                            }
                        } else {
                            console.log('存在');
                        }
            ...
            _submit () { // 提交
                ...
                if (!this.rawData.diseaseName) { // 0：未对照 1已对照未审核 2已审核 3已撤销 4不能对照 ,
        ...
        destroyed () {
            busEvent.$off('baseDataContrast/diseaseControl/common/StandardData.dbClick');
        },
