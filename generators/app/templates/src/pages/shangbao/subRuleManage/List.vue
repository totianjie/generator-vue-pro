...
<script>
    ...
    import {deleteSubRule, audit} from 'src/js/api/shangbao/subRuleManage';
    export default {
        data () {
            return {
                total: 0, // 分页总条数
        ...
        methods: {
            ...
            async handleAudit () {
                ...
                console.log(this.selectList.map(item => item.subRuleId).join(','));
                await audit({subRuleId: this.selectList.map(item => item.subRuleId).join(',')});
                this._refresh();
                this._inttPaging();
            },
            async handleDelete () {
                ...
                await deleteSubRule({subRuleId: this.selectList.map(item => item.subRuleId).join(',')});
                this._refresh();
                this._inttPaging();
            ...
            handleEdit (row) {
                ...
                this.$emit("update:subRuleId", row.subRuleId);
                this.$emit('input', 'add');
