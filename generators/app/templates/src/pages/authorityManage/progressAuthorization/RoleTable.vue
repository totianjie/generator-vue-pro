            ...
            <ul class="tree" v-if="tableData.length > 0">
                <li class="item" v-for="(item, index) in tableData" :key="index">
                    <div :class="['text text-ellipsis',{active:selectedRoleGroup == item.id}]">
                        <i class="el-icon-caret-bottom" :class="['icontext',{active:selectedRoleGroup == item.id}]"  @click="selectRoleGroupItem(item)" v-if="currentSelectGroupIndex == index && hiddenTreeShow"></i>
                    ...
                    <ul class="tree-text" v-show="hiddenTreeShow"  v-if="item.hasOwnProperty('children') && Array.isArray(item.children)" :key="`child_${index}`">
...
<script type="text/ecmascript-6">
    ...
    import { getRole } from "js/api/authorityManage/role";
    ...
    export default {
        ...
        methods: {
            ...
            _reset (data, reset = false) {
                console.log('tag', data);
                if (data.id === -1) {
                    return;
                }
                this.hiddenTreeShow = true;
                this.currentSelectGroupIndex = 0;
                this.showTreeRoleGourp = true;
                getRole({'id': data.id}).then((res) => {
                    ...
                    let list = res.data;
                    if (list.length > 0) {
                        let childrenitem = {roleGroupId: data.id, id: list[0].id};
                        console.log('childrenitem', childrenitem);
                        this.$emit('authSelect', childrenitem);
                    }
                    if (reset) {
                        return;
                    }
            ...
            editInfo () {
                console.log('editInfo', this.selectedRole);
                if (this.selectedRole === -1) {
                    this.$toast('请选择角色后在编辑');
                    return;
                }
                if (this.selectedRole !== -1) {
                    this.$emit('editRoleAuth', this.currentRoleData);
                } else {
                    this.$emit('editRoleGroupAuth', this.currentRoleGroupData);
                }
            },
            refreshTableView () {
                this.selectedRoleGroup = -1;
                ...
                this.hiddenTreeShow = false;
            },
            selectRoleGroupItem (data) {
                try {
                    console.info('selectRoleGroupItem');
                    ...
                    if (data.isRoleGroup) {
                        try {
                            if (this.currentSelectGroupIndex === data.groupIndex_ && this.showTreeRoleGourp) {
                                ...
                                console.info('tag', '11111');
                                return;
                            }
                            // this._reset(this.tableData[0] || {id: -1}, true);
