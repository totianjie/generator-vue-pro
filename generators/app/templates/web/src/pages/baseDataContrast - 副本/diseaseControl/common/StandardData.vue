...
<script>
    ...
    import {getStandardList} from "js/api/baseDataContrast/diseaseControl";
    ...
    export default {
        ...
        mounted () {
            if (this.oldStandardSearchParams.versionId) {
                this._getTableData();
            }
            ...
            busEvent.$on('baseDataContrast/diseaseControl/common/RawDataTableMixins.rowClick', ({instDiseaseName} = {}) => {
                let result = Object.assign({}, this.searchParams, {diseaseName: instDiseaseName});
                this.$emit('update:searchParams', result);
            });
        },
        methods: {
            handleRowClick (row = null) { // 单击
                busEvent.$emit('baseDataContrast/diseaseControl/common/StandardData.standardData', row);
            },
            handleRowDbClick (row = null) { // 双击
                busEvent.$emit('baseDataContrast/diseaseControl/common/StandardData.dbClick', row);
            },
            handleSelection (arr = []) { // 复选框选择  改变
                this.standardDataList = arr;
                busEvent.$emit('baseDataContrast/diseaseControl/common/StandardData.standardDataList', arr);
            },
            ...
            handlerClear () { // 清空查询条件
                ...
                let result = Object.assign({}, {
                    versionId: this.searchParams.versionId,
                    operationCode: '',
                    operationName: ''
                });
                console.log(result);
            ...
            _reset () {
                ...
                busEvent.$emit('baseDataContrast/diseaseControl/common/StandardData.standardDataList', []);
                busEvent.$emit('baseDataContrast/diseaseControl/common/StandardData.standardData', null);
            },
            // 排序
